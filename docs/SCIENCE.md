...
- **4 qubits**, **L = 4 re-uploading blocks** on |0000⟩. Block *l*:
  1. encoding: `RY(λ[l,i] · s[i])` on each qubit *i* — the 4 observation
     features `s` (3 lidar rays + speed, each normalized to [0, 1]) are
     re-uploaded in **every** block;
  2. variational: `RY(θ[l,i,0])` then `RZ(θ[l,i,1])` on each qubit;
  3. entanglement: a CZ ring `CZ(0,1) … CZ(n−1,0)`.
- **Readout:** one Pauli-Z expectation per action, `E_a = ⟨Z_a⟩`, mapped
  through a trainable classical head `Q_a = w[a]·E_a + b[a]`. The output
  scaling is essential: useful Q-values for this task reach the hundreds
  while ⟨Z⟩ ∈ [−1, 1].
- **Trainable parameters, 56 total**, one flat vector `[λ, θ, w, b]`:
  input scalings λ (4×4 = 16, init π), variational angles θ (4×4×2 = 32,
  init U(−0.1, 0.1)), output weights w (4, init 1) and biases b (4, init 0).
...
phases a Z measurement can never see — the gradient of those 4 final-RZ
parameters is *exactly* zero for every input; they can never train. Kept
...
noise model + coupling map of a real 5-qubit device, simulated locally —
makes everything below runnable without an account.)
...
- **Parity at tiny scale, not speedup.** A 56-parameter VQC and a
  ...
  is the point, echoing Chen et al. (2020) and Skolik et al. (2022).
- **No quantum advantage is claimed — or possible here.** Four qubits are
  trivially simulable; training literally ran on a classical simulation of
  the circuit. An advantage claim would need problem classes where a quantum
  model provably captures structure a comparably-sized classical model
  cannot, not a racing toy.
